...
- 홈의 카드 결제 예정은 이번 달과 다음 달을 함께 표시하고 월별 합계를 제공합니다. 카드별 금액을 누르면 일반 결제, 할부 회차, 승인취소를 포함한 해당 결제주기의 전체 지출을 볼 수 있습니다. 이번 달 결제일이 지난 카드는 홈에서 0원으로 표시하되 당시 포함 내역은 계속 확인할 수 있습니다.
- 빠른 입력은 등록된 활성 카드 중 목록 첫 카드를 기본 결제수단으로 선택합니다(카드가 없으면 현금). **고정지출 만들기**에서는 이름·금액·매월 날짜·카테고리·결제수단·메모를 저장하고 수정할 수 있습니다. 지정일에 선택한 카드·현금·계좌의 지출을 월 1회 자동 생성하며, 앱을 늦게 열어도 누락 월을 중복 없이 반영합니다. 수정해도 이미 생성된 과거 거래는 바꾸지 않습니다.
