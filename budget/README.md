...
- 홈의 카드 결제 예정은 이번 달과 다음 달을 함께 표시합니다. 이번 달 결제일이 지난 카드는 0원으로 표시하고, 다음 달은 현재까지 해당 결제주기에 포함된 사용액을 보여줍니다.
- 빠른 입력의 **고정지출 만들기**에서 이름·금액·매월 날짜·카테고리·메모를 저장할 수 있습니다. 지정일에 현금 고정지출을 월 1회 자동 생성하며, 앱을 늦게 열어도 누락 월을 중복 없이 반영합니다.
- 통계의 **총사용액**은 각 카드별 사용액, 현금 고정지출, 현금 일반지출을 분리하고 카드총액·현금총액·총사용액을 함께 표시합니다. 카드가 아닌 계좌 직접지출은 현금총액에 포함됩니다.
