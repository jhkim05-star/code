...
export function validateState(state){
  ...
  for(const rule of state.recurringExpenses){if(!rule.id||!rule.name||!Number.isFinite(+rule.amount)||+rule.amount<=0||!Number.isInteger(+rule.day)||+rule.day<1||+rule.day>31||!parseDate(`${rule.startMonth}-01`)||!Array.isArray(rule.generatedMonths||[]))throw new Error('고정지출 항목을 확인해 주세요.');const payment=rule.paymentMethod||'cash';if(!['cash','account','card'].includes(payment)||payment==='card'&&!state.cards.some(card=>card.id===rule.cardId))throw new Error('고정지출 결제수단을 확인해 주세요.');}
