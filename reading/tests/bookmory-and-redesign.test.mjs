...
test('실제 업로드 Bookmory 파일 전체를 파싱한다',{skip:!process.env.BOOKMORY_SAMPLE},async()=>{
  ...
  assert.equal(parsed.state.books.filter(b=>!b.genre).length,0);assert.equal(parsed.state.books.filter(b=>!b.origin).length,0);assert.equal(parsed.state.books.filter(b=>b.coverUrl).length,0);
