...
export function renderPlanTab(root, params, opt = {}) {
    ...
    function sessionCard(s) {
        return h('.card', null, h('h3', null, '운동시간과 목표'), field('하루 운동시간', stepper({ value: s.plan.sessionMinutes, min: 20, max: 150, step: 5, format: v => `${v}분`, onchange: v => setSetting('plan.sessionMinutes', v) }), '웜업·휴식·기구 준비·카운트다운을 포함합니다.'), field('목표', select(s.plan.goal, [['general', '꾸준한 일반 운동'], ['strength', '근력 중심'], ['hypertrophy', '근비대 중심']], v => setSetting('plan.goal', v))), field('경험 수준', select(s.plan.experience, [['unknown', '아직 설정하지 않음'], ['beginner', '입문'], ['intermediate', '중급'], ['advanced', '숙련']], v => setSetting('plan.experience', v))), switchRow('웜업 세트 포함', '기구와 준비한 동작을 구분합니다. 추정 중량은 확인이 필요해요.', s.plan.warmup, v => setSetting('plan.warmup', v)), field('증량 판단에 필요한 여유 횟수', stepper({ value: s.plan.targetRir, min: 0, max: 5, step: 1, format: v => `RIR ${v}`, onchange: v => setSetting('plan.targetRir', v) }), '본세트 전체가 목표를 채우고 이 여유가 확인된 경우에만 다음 기구 단계를 제안해요.'));
    }
    ...
    function equipmentCard(s) { return h('.card', null, h('h3', null, '가진 기구'), h('p.hint', null, '아무것도 선택하지 않으면 전부 허용합니다.'), h('.chips', null, ...EQUIPMENT.map(eq => h('button.chip', { 'aria-pressed': s.plan.equipment.includes(eq), onclick: () => { const next = new Set(settings().plan.equipment); next.has(eq) ? next.delete(eq) : next.add(eq); setSetting('plan.equipment', [...next]); draw(); } }, eq)))); }
    function avoidCard() {
