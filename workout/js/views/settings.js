...
function addCustomSheet(redraw) {
    modal(close => {
        let customMachines = [];
        const machineStatus = h('p.hint', null, '머신 종목이면 실제 필요한 머신을 골라 주세요.');
        const name = h('input', { type: 'text', maxLength: 160, placeholder: '운동 이름' }), group = select('chest', GROUPS.map(g => [g.id, g.name]), () => { }), equip = select('덤벨', [...EQUIPMENT, '머신'].map(e => [e, e]), () => { }), tier = select('2', [['1', '주요 종목'], ['2', '보조 종목'], ['3', '고립·마무리']], () => { }), measure = select('reps', [['reps', '횟수(회)'], ['duration', '유지시간(초)']], () => { }), loadBasis = select('total', [['total', '전체 표기 중량'], ['per_hand', '한쪽 덤벨'], ['stack', '머신·케이블 표시'], ['assistance', '보조중량 · 낮을수록 실제 부하 증가'], ['bodyweight', '맨몸'], ['added', '추가 중량']], () => { }), sets = h('input', { type: 'number', min: 1, max: 20, value: 3 }), reps = h('input', { type: 'number', min: 1, max: 600, value: 12 }), rest = h('input', { type: 'number', min: 0, max: 900, value: 90 });
        return h('div', null, h('h3', null, '사용자 종목 추가'), field('이름', name), field('부위', group), field('기구', equip), h('button.btn-block', { onclick: () => pickMachines(customMachines, ids => { customMachines = ids; machineStatus.textContent = ids.length ? `${ids.length}개 · ${machineNames(ids).join(', ')}` : '선택한 머신이 없어요.'; }) }, '필요한 머신 선택'), machineStatus, field('분류', tier), field('측정 방식', measure), field('중량 표기 방식', loadBasis), h('.btn-row', null, field('세트', sets), field('횟수·초', reps)), field('본세트 휴식(초)', rest), h('button.btn-block.btn-primary', { onclick: async () => {
                if (!name.value.trim())
                    throw new Error('이름을 입력해 주세요.');
                if (equip.value === '머신' && !customMachines.length)
                    throw new Error('머신 종목에 필요한 헬스장 머신을 골라 주세요.');
                addCustomExercise({ id: uid('cx'), name: name.value.trim(), group: group.value, equip: equip.value, requiredEquipment: equip.value === '머신' ? [] : [equip.value], machineIds: equip.value === '머신' ? customMachines : [], tier: Number(tier.value), measure: measure.value, loadBasis: loadBasis.value, sets: finite(sets.value, 1, 20, '세트', { integer: true }), reps: finite(reps.value, 1, 600, '목표', { integer: true }), rest: finite(rest.value, 0, 900, '휴식', { integer: true }), tempo: measure.value === 'duration' ? 1 : 3, compound: false, warmupEligible: false });
                await flush();
                close();
                redraw();
            } }, '종목 추가'));
