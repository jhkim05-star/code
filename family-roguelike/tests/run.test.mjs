...
import { REWARD_CARDS, STARTER_DECK } from '../src/game/cards.ts';
import { FLOOR_ENEMIES } from '../src/game/enemies.ts';
import { chooseReward, endRunTurn, FINAL_FLOOR, playRunCard, startRun } from '../src/game/run.ts';
...
test('winning a normal floor requires exactly one reward before the next battle', () => {
  ...
  const reward = playRunCard(before, 0);
  ...
  assert.equal(chooseReward(reward, REWARD_CARDS.length), reward);
  ...
  assert.equal(next.deck.at(-1), REWARD_CARDS[1]);
...
test('four rewards build the deck and the fifth floor ends at the boss', () => {
  ...
  for (let floor = 1; floor <= FINAL_FLOOR; floor++) {
    run = playRunCard(oneHitFromWin(run), 0);
